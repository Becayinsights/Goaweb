...
# Las secciones van en el orden en que aparecen al bajar. Tratamientos no es un
# salto a una sección sino un desplegable, y por eso es lo único que en móvil se
# queda arriba, en la isla: la barra de abajo es para saltar de sección.
SECCIONES = """      <a href="/#resultados">Antes y después</a>
      <a href="/#especialidades">Especialidades</a>
      <a href="/#sobre-mi">Sobre mí</a>
      <a href="/#faq">FAQ</a>"""
DROP = ('      <button class="drop" type="button" aria-expanded="false" aria-controls="mega">'
        'Tratamientos <span class="caret" aria-hidden="true">›</span></button>')
# En la isla el desplegable ocupa su sitio por orden de scroll, entre
# especialidades y sobre mí.
LINKS = "\n".join(SECCIONES.split("\n")[:2] + [DROP] + SECCIONES.split("\n")[2:])
...
{LINKS}
    ...
    <div class="hact">
      <a class="btn btn-solid btn-sm cta-top" href="/#contacto"><span class="cta-larga">Pedir valoración</span><span class="cta-corta">Valoración</span></a>
    </div>
  </header>
</div>"""
...
def menu_js():
  ...
  function colocar(){
    ...
    mega.style.maxHeight=(innerHeight - r.bottom - 24) + "px";
    if(innerWidth > 980){ mega.style.left=r.left + "px"; mega.style.width=r.width + "px"; }
    else { mega.style.left=mega.style.width=""; }
  }
  function abrir(v){
    if(v) colocar();
    mega.hidden=!v;
  ...
  var conRaton = matchMedia("(hover:hover) and (pointer:fine)").matches;
  var cierre;
  function entrar(){ clearTimeout(cierre); abrir(true); }
  function salir(){ clearTimeout(cierre); cierre=setTimeout(function(){ abrir(false); }, 180); }
  /* Con ratón y ancho de escritorio se abre al pasar por encima y el clic lleva
     a la sección; en móvil, donde el botón vive en la isla junto a la llamada,
     el toque abre y cierra. Mirando solo el puntero, en un equipo con ratón y
     ventana estrecha el botón navegaba y el panel no llegaba a abrirse. */
  drops.forEach(function(b){
    b.addEventListener("click",function(e){
      e.stopPropagation();
      if(conRaton && innerWidth > 980){ abrir(false); location.href = "/#tratamientos"; }
      else { abrir(mega.hidden); }
    });
    if(conRaton){
      b.addEventListener("mouseenter", function(){ if(innerWidth > 980) entrar(); });
      b.addEventListener("mouseleave", function(){ if(innerWidth > 980) salir(); });
      b.addEventListener("focus", function(){ if(innerWidth > 980) entrar(); });
    }
  });
  if(conRaton){
    mega.addEventListener("mouseenter", function(){ if(innerWidth > 980) entrar(); });
    mega.addEventListener("mouseleave", function(){ if(innerWidth > 980) salir(); });
  }
  ...
  /* Dos estados y no un bucle: con la página quieta la marca se pliega en el
     isologo y ahí se queda; al volver a moverse, se abre en GOA. */
  ...
  ["scroll","wheel","pointerdown","keydown","touchstart"].forEach(function(ev){
    addEventListener(ev,function(){ ultimo=Date.now(); island.classList.remove("reposo"); },{passive:true});
  });
  island.addEventListener("animationend",function(e){
    if(e.animationName==="goIn") island.classList.remove("intro");
  });
  setInterval(function(){
    if(reduce.matches || document.hidden || Date.now()-ultimo < 2500) return;
    if(island.classList.contains("intro")) return;
    island.classList.add("reposo");
  }, 250);
"""
